// Pobiera ceny z Trade Marketu Wynncrafta (WynnVentory API v2, https://wynnventory.com) i zapisuje src/item-prices.json.
...
console.log(`Saved prices for ${Object.keys(items).length} items (${rows} ranking rows, ${isoDay(start)}..${isoDay(end)}) to ${OUTPUT.pathname}`);
